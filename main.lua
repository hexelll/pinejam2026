...
local function launchAttack(state,w)
    ...
    -- if big bombs :
    if math.random()>0.5 then -- TEMP TEMP
        ...
        for i=1,nbShots do 
            ...
            state.shots[state.shotsId] = {
                x= x,
                y= y,
                r= math.max( 0.03 , math.min( 0.4 , randomFloat(0.01,d) )),
                bombingTime = os.clock()+1+delay,
                startTime = os.clock()+delay
            }
            state.shotsId = state.shotsId + 1
        end

    -- if small line of bombs :
    else

        local size = 0.03

        local x,y =  w.x-w.r+size,w.y-w.r+size
        local dx,dy =  randomFloat(0.02,0.07),randomFloat(0.02,0.07)
        ...
        state.shots[state.shotsId] = {
                ...
                y= y,
                r= size,
                bombingTime = os.clock()+1 +nb*delay,
                startTime = os.clock() +nb*delay
            }
        state.shotsId = state.shotsId + 1
        nb = nb +1

        while ( w.x-w.r+size<=x and x<=w.x+w.r-size and w.y-w.r+size<=y and y<=w.y+w.r-size) do
            x,y = x+dx, y+dy
            state.shots[state.shotsId] = {
                x= x,
                y= y,
...
gameHandler:addTask(function (state)
    ...
    if ( os.clock() - state.bombWarningTime > delayBombs )then
        ...
        state.bombsWarnings[state.warningId] = {
            ...
            r= math.max( 0.05 , math.min( 0.4 , 0.05+math.random()*state.dangerLevel/10 )) ,
